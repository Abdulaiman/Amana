...
const AdminDashboard = () => {
    ...
    const handleConfirmPayout = async (id) => {
        if (!window.confirm('Confirm this payout has been transferred?')) return;
        try {
            await api.put(`/admin/withdrawals/${id}/confirm`);
            loadData();
            alert('Payout Confirmed & Wallet Deducted');
        } catch (e) {
            console.error(e);
            alert('Payout Confirmation Failed');
        }
    };
    ...
    const handleRejectVendor = async (id) => {
        if (!window.confirm('Reject this vendor verification?')) return;
        try {
                                            ...
                                            {filteredWithdrawals.map(req => (
                                                <tr key={req._id} className="payouts-row group">
                                                    ...
                                                    <td className="td-cell pr text-right">
                                                        {req.status === 'pending' && (
                                                            <button 
                                                                onClick={() => handleConfirmPayout(req._id)}
                                                    ...
                                                    <td className="td-cell pr text-right">
                                                        {vendor.verificationStatus === 'pending' && vendor.isProfileComplete && (
                                                            <div className="action-btns">
                                                                <button onClick={() => handleVerifyVendor(vendor._id)} className="confirm-transfer-btn">
                                                                    <Check size={14} /> Verify
                                                                </button>
                                                                <button onClick={() => handleRejectVendor(vendor._id)} className="reject-btn">
                                                                    <X size={14} /> Reject
                                                                </button>
                                                            </div>
                                                        )}
